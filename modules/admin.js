...
// Helper function to get Winden user ID from Pterodactyl ID
async function getWindenUserId(pterodactylId, db) {
  try {
    console.log('Looking up Winden ID for Pterodactyl ID:', pterodactylId);
    
    // Check if the mapping exists
    const userIds = await db.get('users') || [];
    for (const userId of userIds) {
      for (const [key, value] of await db.iterate()) {
        if (key.startsWith('users-') && value === pterodactylId) {
          return key.replace('users-', '');
        }
      }
    }
    
    console.log('No Winden user found for Pterodactyl ID:', pterodactylId);
    return null;
  } catch (error) {
    console.error("Error getting Winden user ID:", error);
    return null;
  }
}
...
app.get("/api/users/:id/coins", async (req, res) => {
  ...
  try {
    ...
    // First check if this is already a Winden user ID
    const directCheck = await db.get("users-" + pterodactylId);
    let userId;

    if (directCheck) {
      // The ID provided was a Winden user ID
      userId = pterodactylId;
      console.log('ID was already a Winden user ID:', userId);
    } else {
      // Try to find the Winden user ID from Pterodactyl ID
      userId = await getWindenUserId(pterodactylId, db);
      console.log('Looked up Winden user ID:', userId);
    }
